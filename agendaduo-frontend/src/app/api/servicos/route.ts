...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.from(TABLES.servicos).select('*').eq('clinica_id', cid).eq('ativo', true).order('nome', { ascending: true });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const db = getSupabase();
    const { data, error } = await db.from(TABLES.servicos).insert(payload).select().single();
    if (error) return err(error.message, 500);
