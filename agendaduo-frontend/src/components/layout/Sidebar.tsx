...
import { Calendar, Users, Briefcase, Activity, DollarSign, MessageCircle, Settings, LayoutDashboard, ChevronRight, CreditCard, Shield, Clipboard, LifeBuoy } from 'lucide-react';
...
const navItems = [
  ...
  { name: 'Suporte', href: '/app/suporte', icon: LifeBuoy },
...
export function Sidebar({ isOpen = false, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside className={cn(
      ...
      {!isMasterPath && loggedRole === 'admin' && profissionais.filter(p => p.id !== localStorage.getItem('agendaduo_admin_profissional_id')).length > 0 && (
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {isMasterPath ? (
          <Link
            ...
            onClick={onClose}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-md shadow-blue-200 group relative"
          >
            <Shield className="w-4.5 h-4.5 shrink-0 transition-transform duration-200 group-hover:scale-110 text-white" />
            <span className="flex-1 text-xs">Painel Master</span>
          </Link>
        ) : (
          navItems
      ...
      {!isMasterPath && typeof window !== 'undefined' && localStorage.getItem('agendaduo_is_master') === 'true' && (
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 bg-slate-50 rounded-xl">
          <div className="relative shrink-0">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white text-xs font-bold">
              {isMasterPath ? 'M' : (userRole === 'admin' ? 'A' : (profissionais.find(p => p.id === selectedProfissionalId)?.nome?.charAt(0) || 'P'))}
            </div>
            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-slate-700 truncate">
              {isMasterPath ? 'Master Admin' : (userRole === 'admin' ? `Adm (${adminName})` : (profissionais.find(p => p.id === selectedProfissionalId)?.nome || 'Profissional'))}
            </p>
            <p className="text-[10px] text-slate-400 truncate">
              {isMasterPath ? 'juanestevam19@outlook.com' : (userRole === 'admin' ? adminSpec : (profissionais.find(p => p.id === selectedProfissionalId)?.especialidade || '-'))}
            </p>
          </div>
        </div>
      </div>
